...
const createSchema = z.object({ ...baseSchema, password: z.string().min(6, 'Min 6 characters') });
...
interface Props { mode: 'create' | 'edit'; user?: User; onClose: () => void; }
...
export function UserDialog({ mode, user, onClose }: Props) {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors, isSubmitting } } = useForm<CreateForm | EditForm>({
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <FormField label="First Name" error={errors.firstName?.message} required>
              <Input {...register('firstName')} placeholder="John" className="bg-muted/50" />
            </FormField>
            <FormField label="Last Name" error={errors.lastName?.message} required>
              <Input {...register('lastName')} placeholder="Doe" className="bg-muted/50" />
            </FormField>
          </div>

          <FormField label="Email Address" error={errors.email?.message} required>
            <Input {...register('email')} type="email" placeholder="john@example.com" className="bg-muted/50" />
          </FormField>
