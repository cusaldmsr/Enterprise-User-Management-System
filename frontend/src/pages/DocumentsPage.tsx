...
import { FileText, Upload, FileImage, FileArchive, Download, FolderOpen, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center justify-center flex-shrink-0 w-10 h-10 rounded-xl bg-cyan-500/15">
          <FolderOpen size={20} className="text-cyan-500" />
        </div>
        <div>
          <h2 className="text-xl font-bold text-foreground">Documents</h2>
          <p className="text-sm text-muted-foreground">Upload and manage files stored on AWS S3</p>
        </div>
      </div>
      ...
      >
        ...
        <div className={cn(
          ...
          {uploadMutation.isPending
            ...
            : <Upload size={22} className={dragOver ? 'text-cyan-500' : 'text-muted-foreground'} />
          }
        </div>
        <p className="mb-1 font-medium text-foreground">
          {uploadMutation.isPending ? 'Uploading to S3...' : dragOver ? 'Drop to upload' : 'Drag & drop files here'}
        </p>
        <p className="text-sm text-muted-foreground">PDF, DOCX, PNG, JPEG — max 20 MB</p>
      </div>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="py-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="muted" className="ml-1 text-xs">{total}</Badge>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="text-center py-14">
              <FileText size={40} className="mx-auto mb-3 text-muted-foreground/20" />
              <p className="text-sm text-muted-foreground">No documents uploaded yet</p>
              <p className="mt-1 text-xs text-muted-foreground/60">Drop a file above to get started</p>
            </div>
          ) : (
            <ScrollArea className="max-h-[500px]">
              <div className="divide-y divide-border">
                {docs.map((doc) => (
                  ...
                  >
                    {/* File icon */}
                    <div className="flex items-center justify-center flex-shrink-0 w-10 h-10 border rounded-xl bg-muted border-border">
                      {FILE_ICON[doc.fileType] ?? <FileText size={18} className="text-muted-foreground" />}
                    </div>

                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate text-foreground">{doc.originalName}</p>
